...
const solution = (req,res) => {
  ...
  //jb tk destination addresses poori ni ho jati chalte raho
  while (calculations.customersRemaining > 0) {
    //agar prevDistance =0 hai tab jao
     ...
     */
      ...
      if (
        ...
        if (calculations.jump < distances.length - 2) {
          
          current = distances[calculations.lastAllocated + calculations.jump].elements

          for (let i = 0; i < current.length; i += 1) {

            if (
              (current[close].distance.value
                >= current[i].distance.value) && (i !== calculations.lastAllocated)
            ) {
              close = i;
            }
          }

          testRouteObject.customer = customers[close];
          if (calculations.lastAllocated + calculations.jump === customers.length) {
            close = 0
            testRouteObject.customer = customers[close];
            calculations.lastAllocated += 1
          }

          testRouteObject.path = `${current[close].distance.text} from ${customers[calculations.lastAllocated]} to ${customers[close]}.`
          testRouteObject.deliveryCharges = calculateCharges(current[close].distance.value);
          calculations.currentRoute.push(testRouteObject);
          calculations.prevDistance += current[close].distance.value
          calculations.jump += 1
    ...
    if (calculations.customersRemaining < 1) {
      ...
      calculations.deliveryRoutes.push({ totalLength: `${Number(calculations.prevDistance * 0.000621371192).toFixed(1)} miles from Store.`, route: calculations.currentRoute });
      /* Set the current route be empty */
      calculations.currentRoute = [];
    }
  }
